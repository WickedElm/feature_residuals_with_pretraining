...
def accuracy(predicted_labels=None, true_labels=None, metrics_dir='.', is_training=True, log_to_disk=True):
    if not is_training:
        metrics_file_path = metrics_dir + '/test_metrics.csv'
    else:
        metrics_file_path = metrics_dir + '/train_metrics.csv'

    metric = sklearn.metrics.accuracy_score(true_labels, predicted_labels)
    ...
    if log_to_disk:
        ...
        metrics_df['accuracy'] = [metric]
        metrics_df.to_csv(metrics_file_path, index=False)

    return metric

def false_alarm_rate(predicted_labels=None, true_labels=None, metrics_dir='.', is_training=True, log_to_disk=True):
    if not is_training:
        metrics_file_path = metrics_dir + '/test_metrics.csv'
    else:
        metrics_file_path = metrics_dir + '/train_metrics.csv'
    ...
    if FP[1] + TN[1] == 0:
        ...
        metric = FP[1] / (FP[1] + TN[1])

    print('False Alarm Rate:  ' + str(metric))
    ...
    if log_to_disk:
        ...
        metrics_df['false_alarm_rate'] = [metric]
        metrics_df.to_csv(metrics_file_path, index=False)
...
def generate_confusion_matrix(predicted_labels, true_labels, normalize=None):
        cm = sklearn.metrics.confusion_matrix(true_labels, predicted_labels, labels=[0,1], normalize=normalize)
        return cm
...
def plot_confusion_matrix(predicted_labels=None, true_labels=None, metrics_dir='.', prefix='', is_training=True, title='Confusion Matrix', target_names=None):
    if not is_training:
        cm_file_path = metrics_dir + f'/{prefix}_cm.png'
        cm_norm_file_path = metrics_dir + f'/{prefix}_cm_norm.png'
    else:
        cm_file_path = metrics_dir + f'/{prefix}_cm.png'
        cm_norm_file_path = metrics_dir + f'/{prefix}_cm_norm.png'
