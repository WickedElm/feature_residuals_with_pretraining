...
class NetflowConferenceSarhanFormatWithCacheDataModule(pl.LightningDataModule):
    def __init__(self, opt=None, data_path='./', batch_size=32, total_rows_threshold=200000, weighted=False, load_from_disk=False, load_data_path='tmp_ds_data', prefix='', reserve_type='', oversampling_multiplier=1.0):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            self.ds_train = SarhanFormatWithCacheNetflowDataset(ds.training_data, self.transform, class_weights=class_weights)
            self.ds_val = SarhanFormatWithCacheNetflowDataset(ds.validation_data, self.transform)
            self.ds_test = SarhanFormatWithCacheNetflowDataset(ds.test_data, self.transform)
