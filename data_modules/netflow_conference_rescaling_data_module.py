...
from data_modules.torch_datasets import SScalingNetflowDataset
import torch
import torchmetrics
...
class NetflowConferenceRescalingDataModule(pl.LightningDataModule):
    def __init__(self, opt=None, data_path='./', batch_size=32, total_rows_threshold=500000, weighted=False, load_from_disk=False, load_data_path='tmp_ds_data', prefix='', reserve_type='', oversampling_multiplier=1.0):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if not self.data_loaded:
            ...
            ds.save_indices()
            ds.write_indices_to_disk(os.getcwd(), f'{self.cfg.general.save_prefix}_{self.cfg.general.experiment}')
            ...
            self.ds_train = SScalingNetflowDataset(ds.training_data, self.transform, class_weights=class_weights)
            self.ds_val = SScalingNetflowDataset(ds.validation_data, self.transform)
            self.ds_test = SScalingNetflowDataset(ds.test_data, self.transform)
