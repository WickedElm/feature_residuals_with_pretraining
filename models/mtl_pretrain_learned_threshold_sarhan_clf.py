...
class MtlPretrainLearnedThresholdSarhanClf(pl.LightningModule):
    ...
    def forward(self, X, L=None, S=None):
        ...
        thresholds = self.threshold_layer(S)
        T = torch.where(S > thresholds, S, torch.zeros_like(S))
        ...
        # Return all outputs
        return P, torch.tensor(predictions, dtype=int), L, S, T
    ...
    def configure_optimizers(self):
        ...
        full_training_optimizer = torch.optim.Adam([
            {'params':self.mlp_layers.parameters()},
            {'params':self.threshold_layer.parameters()}
            ], lr=float(self.hparams['clf_adadelta_lr']))
    ...
    def validation_step(self, batch_dc, batch_idx):
        ...
        if self.current_epoch < self.num_pretraining_epochs:
            ...
            y_pred, predictions, L, S, T = self.forward(x, L=None, S=None)
            loss = self.loss_function(L, x, y_pred, label)
            self.log(f'val/{self.threshold_name}/loss', loss, on_step=False, on_epoch=True)
            self.log(f'val/{self.threshold_name}/clf_loss', loss, on_step=False, on_epoch=True)
            return {'val_loss':loss, 'y_pred':predictions, 'y_true':label, 'L':L, 'S':S, 'T':T}

    def validation_epoch_end(self, val_step_outputs):
        # NOTE:  This is still using training data here
        X = torch.tensor(self.trainer.datamodule.ds_train.original_X.iloc[:,1:-1].values).float().to(self.device)
        ...
        y_pred, predictions, L, S, T = self.forward(X)
    ...
    def test_step(self, batch_dc, batch_idx):
        ...
        return {'test_loss':loss, 'y_pred':predictions, 'y_true':label, 'L':L, 'S':S, 'T':T}
