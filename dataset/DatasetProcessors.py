...
class CleanProtocolColumn(DatasetProcessor):
    ...
    def map_unknown_protocols(self, val):
        ...
        if type(val) == int and val not in [6, 17, 132, 1, 2, 50, 47, 94, 0, 41, 55, 4, 44, 43, 59, 60, 54, 91, 89]:
            print(val)
            val = 'other'
...
class ConsolidateProtocols(DatasetProcessor):
    def __init__(self, target_column='protocol', values_to_consolidate=[], new_value=-1):
        self.target_col = target_column
        self.values_to_consolidate = values_to_consolidate
        self.new_value = new_value

        self.protocol_map = dict()
        for val in self.values_to_consolidate:
            self.protocol_map[val] = self.new_value

    def process(self, df):
        df[self.target_col] = df[self.target_col].map(self.protocol_map)
        return df
...
class AddAnnotationColumn(DatasetProcessor):
    def __init__(self, target_col='', value=''):
        super().__init__()
        self.target_col = target_col
        self.value = value

    def process(self, df):
        df[self.target_col] = self.value
        return df
...
class PrepareAttackBenignLabel(DatasetProcessor):
    ...
    def process(self, df):
        df[self.target_col] = df[self.target_col].apply(lambda x: 0 if x.lower() == 'benign' else 1)
        return df
...
class MarkCommsTypeUsingServiceColumn(DatasetProcessor):
    def __init__(self, new_column_name='communication_type', service_column_name='service'):
        super().__init__()
        self.service_column_name = service_column_name
        self.new_column_name = new_column_name
        self.port_map = {
            'ftp':'ftp',
            'ftp-data':'ftp',
    ...
    def map_ports(self, service):
        # These two checks allow us to use either port to 
        # indicate the communication type for a given flow
        # If not in our map, we really don't care about it
        if service in self.port_map.keys():
            return service 
    ...
    def process(self, df):
        df[self.new_column_name] = df.apply(lambda x: self.map_ports(x[self.service_column_name]), axis=1)
        df[self.new_column_name] = df[self.new_column_name].map(self.port_map)
        comms_type = pd.CategoricalDtype(set(self.port_map.values()))
        df[self.new_column_name] = df[self.new_column_name].astype(comms_type)
        return df
